...
if (!defined('POSTS_BRIDGE_GS_STORE_SECRET')) {
    define('POSTS_BRIDGE_GS_STORE_SECRET', 'wp-bridge-store-secret');
}

class Google_Sheets_Store extends Singleton
{
    private const store_name = 'posts_bridge_gs_store';
    ...
    {
        $secret = substr(POSTS_BRIDGE_GS_STORE_SECRET, 0, $len);

        while (strlen($secret) < $len) {
            $secret .= substr(
                POSTS_BRIDGE_GS_STORE_SECRET,
                0,
                $len - strlen(POSTS_BRIDGE_GS_STORE_SECRET)
            );
        }
