...
export default function TemplatesProvider({ children }) {
  ...
  const fetchConfig = useCallback(
    (template) => {
      return apiFetch({
        path: `posts-bridge/v1/${addon}/templates/${template}`,
      })
        .then((config) => {
          if (!template) return;
          setConfig(config);
        ...
        .catch(() =>
          setError("error", __("Template config load error", "posts-bridge"))
        );
    },
    [addon]
  );
  ...
  const submit = useCallback(
    ({ fields }) => {
      ...
      return apiFetch({
        ...
        .catch(() => setError(__("Template submit error", "posts-bridge")))
  ...
  const reset = useCallback(() => {
    ...
    return apiFetch({
      ...
      .catch(() => setError(__("Template reset error", "posts-bridge")))
      .finally(() => setLoading(false));
  }, [addon, template]);
