...
export default function TemplateWizard({ onSubmit }) {
  const [tab] = useTab();
  ...
  const fetchOptions = useCallback(
    (backend, credential = {}) => {
      ...
      apiFetch({
        path: `posts-bridge/v1/${tab}/templates/${template}/options`,
        method: "POST",
  ...
  return (
    <div style={{ width: "575px", minHeight: "125px" }}>
      ...
      {group === "backend" && fetchError && (
        <div style={{ marginBottom: "10px" }}>
          <Notice status="error" politeness="assertive" isDismissible={false}>
            {__("Unable to fetch data from the backend", "posts-bridge")}
          </Notice>
        </div>
      )}
      <Step
      ...
      {authError && (
        <div style={{ marginBottom: "10px" }}>
          <Notice status="error" politeness="assertive" isDismissible={false}>
            {__("The credential cannot be authorized", "posts-bridge")}
          </Notice>
        </div>
      )}
      {needsAuth && (
        <div style={{ margin: "10px 0" }}>
          <Notice
            ...
            actions={[
              {
                label: __("Authorize", "posts-bridge"),
                onClick: authorize,
                variant: "secondary",
                size: "compact",
              },
