...
/**
 * Registers Airtable http defaults.
 *
 * @param array $setting Http setting data.
 *
 * @return array
 */
function posts_bridge_airtable_register_http_defaults( $setting ) {
	$addon = PBAPI::get_addon( 'airtable' );
	if ( ! $addon->enabled ) {
		return $setting;
	}

	$backend_name = 'Airtable API';
	$backend_url  = 'https://api.airtable.com';

	$credential_name   = 'Airtable Token';
	...
	function ( $settings, $group ) {
		...
		foreach ( $settings as &$setting ) {
			if ( 'http' === $setting['name'] ) {
				$setting['default'] = posts_bridge_airtable_register_http_defaults( $setting['default'] );
			}
		}
...
add_filter(
	...
	function ( $data ) {
		...
		return posts_bridge_airtable_register_http_defaults( $data );
	},
	20,
	1,
);
